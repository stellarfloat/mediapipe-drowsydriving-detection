...
def mark_index_image(img_path: str, output_path: str):
    mp_drawing = mp.solutions.drawing_utils
    mp_face_mesh = mp.solutions.face_mesh
    ...
    min_tracking_confidence=0.65) as face_mesh:
        ...
        if results.multi_face_landmarks:
            ...
            for i in range(len(landmark_list)):
                pos = (int(landmark_list[i][0]), int(landmark_list[i][1]))
                cv2.putText(image, f'{i}', pos, cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255,255,255), 1, cv2.LINE_AA)
